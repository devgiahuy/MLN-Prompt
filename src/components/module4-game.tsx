...
export default function Module4Game() {
  ...
  const [slottedCards, setSlottedCards] = useState<{ [key: number]: any }>({ 1: null, 2: null, 3: null, 4: null });
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        console.log(`Trạng thái kết nối Realtime cho Slot ${currentSlot}:`, status);
      });
  ...
  const handleLockAnswer = () => {
    ...
    const selectedOption = currentData.options.find(o => o.id === selectedOptionId);
  ...
  return (
    <section className={`min-h-screen bg-neutral-950 text-green-500 font-mono py-20 px-6 relative overflow-hidden ${isGlitching ? 'animate-pulse bg-red-950' : ''}`}>
        ...
        {isGlitching && (
          ...
          >
            <div className="text-center p-8 bg-black/80 border-2 border-red-500 rounded-2xl max-w-2xl">
              ...
              <h2 className="text-5xl font-black text-red-500 mb-4 tracking-widest">FATAL ERROR</h2>
              <p className="text-2xl text-red-300">Vi phạm nguyên lý Triết học cơ bản!</p>
              <p className="text-red-400 mt-2">Dữ liệu bị ảo giác. Bắt buộc vote lại.</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-12">
        ...
        <div className="flex-1 space-y-8">
          <div className="mb-12 border-b border-green-900 pb-6">
            <h2 className="text-4xl font-black tracking-widest uppercase mb-2">Jailbreak AI</h2>
            <p className="text-green-700">System Override Protocol - Phase {Math.min(currentSlot, 4)}/4</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[1, 2, 3, 4].map((slotIndex) => {
              ...
              const title = GAME_DATA.find(d => d.slot_id === slotIndex)?.title;
              
              return (
                <div 
                  key={slotIndex} 
                  className={`p-6 rounded-2xl border-2 transition-all duration-500 ${
                    card ? 'bg-green-950 border-green-500' : 
                    isActive ? 'border-dashed border-green-400 bg-green-950/20' : 'border-dashed border-neutral-800 bg-neutral-900/50'
                  } h-40 flex flex-col justify-center relative`}
                >
                  <div className="text-sm text-green-700 font-bold mb-2 uppercase">{title}</div>
          ...
          {currentSlot > 4 && (
            ...
            >
              ...
              <div className="text-lg font-mono font-medium mt-4">The Ultimate Master Prompt Framework generated.</div>
            </motion.div>
          )}
        </div>
        ...
        {currentSlot <= 4 && currentData && (
          <div className="w-full lg:w-[450px] flex flex-col gap-8">
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-8 flex flex-col items-center text-center">
              ...
              <p className="text-neutral-500 text-sm mb-6">Quét mã QR bằng điện thoại để Vote</p>
              <div className="bg-white p-4 rounded-xl mb-4">
                {hostUrl ? (
                  <QRCodeSVG value={`${hostUrl}/vote?slot=${currentSlot}`} size={160} />
                ) : (
                  <div className="w-[160px] h-[160px] bg-neutral-200 animate-pulse rounded-lg" />
                )}
              </div>
              <p className="text-xs text-neutral-600 break-all">{hostUrl}/vote?slot={currentSlot}</p>
            </div>
              ...
              <div className="space-y-4">
                {currentData.options.map((opt) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-green-300 z-10">{opt.text}</div>
                    ...
                    <div className="ml-auto font-bold text-lg z-10 pl-4">{votesCount[opt.id as keyof typeof votesCount]}</div>
                  </motion.div>
                ))}
              </div>
